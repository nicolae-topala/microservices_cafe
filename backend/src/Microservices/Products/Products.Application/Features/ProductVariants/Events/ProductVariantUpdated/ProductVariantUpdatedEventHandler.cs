...
namespace Products.Application.Features.ProductVariants.Events.ProductVariantUpdated;
    ...
    public async Task Handle(ProductVariantUpdatedEvent notification, CancellationToken cancellationToken)
    {
        ...
        if (product is null)
        {
            logger.LogWarning("Product with ID {ProductId} not found for event {EventName}.",
                notification.ProductId, nameof(ProductVariantUpdatedEvent));
            return;
        }
        ...
        if (!success)
        {
            logger.LogError("Failed to update product {ProductId} in Elasticsearch after variant {VariantId} update",
                product.Id, notification.ProductVariantId);
            return;
        }

        logger.LogInformation("Successfully updated product {ProductId} in Elasticsearch after variant {VariantId} update",
            product.Id, notification.ProductVariantId);
    }
}
